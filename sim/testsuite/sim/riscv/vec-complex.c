...
# output: == test: check_pv_cplxmul_real -> success, nr. of errors: 0\n
# output: == test: check_pv_cplxmul_img -> success, nr. of errors: 0\n
# output: == test: check_pv_complex -> success, nr. of errors: 0\n
...
// This testbench checks the basic functionality of complex instructions for
// GAP9 on
...
void check_pv_cplxmul_real (testresult_t *result, void (*start) (),
                            void (*stop) ());
void check_pv_cplxmul_img (testresult_t *result, void (*start) (),
                           void (*stop) ());
void check_pv_complex (testresult_t *result, void (*start) (),
                       void (*stop) ());
...
void
check_pv_cplxmul_real (testresult_t *result, void (*start) (), void (*stop) ())
{
  unsigned int i;
  unsigned int res;
  ...
  for (i = 0; i < N; i++)
    {
      ...
      check_uint32 (result, "pv.cplxmul.h.r", cplxmul_opCr_[i],
                    cplxmul_opCResr_[i]);
    }
  ...
  for (i = 0; i < N; i++)
    {
      asm volatile(
          "pv.cplxmul.h.r.div2 %[c], %[a], %[b]\n"
          : [c] "+r"(cplxmul_opCr_div2[i])
          : [a] "r"(cplxmul_opAr_div2[i]), [b] "r"(cplxmul_opBr_div2[i]));

      check_uint32 (result, "pv.cplxmul.h.r.div2", cplxmul_opCr_div2[i],
                    cplxmul_opCResr_div2[i]);
    }
  ...
  for (i = 0; i < N; i++)
    {
      asm volatile(
          "pv.cplxmul.h.r.div4 %[c], %[a], %[b]\n"
          : [c] "+r"(cplxmul_opCr_div4[i])
          : [a] "r"(cplxmul_opAr_div4[i]), [b] "r"(cplxmul_opBr_div4[i]));

      check_uint32 (result, "pv.cplxmul.h.r.div4", cplxmul_opCr_div4[i],
                    cplxmul_opCResr_div4[i]);
    }
  ...
  for (i = 0; i < N; i++)
    {
      asm volatile(
          "pv.cplxmul.h.r.div8 %[c], %[a], %[b]\n"
          : [c] "+r"(cplxmul_opCr_div8[i])
          : [a] "r"(cplxmul_opAr_div8[i]), [b] "r"(cplxmul_opBr_div8[i]));

      check_uint32 (result, "pv.cplxmul.h.r.div8", cplxmul_opCr_div8[i],
                    cplxmul_opCResr_div8[i]);
    }
}
...
void
check_pv_cplxmul_img (testresult_t *result, void (*start) (), void (*stop) ())
{
  unsigned int i;
  unsigned int res;
  ...
  for (i = 0; i < N; i++)
    {
      ...
      check_uint32 (result, "pv.cplxmul.h.i", cplxmul_opCi_[i],
                    cplxmul_opCResi_[i]);
    }
  ...
  for (i = 0; i < N; i++)
    {
      asm volatile(
          "pv.cplxmul.h.i.div2 %[c], %[a], %[b]\n"
          : [c] "+r"(cplxmul_opCi_div2[i])
          : [a] "r"(cplxmul_opAi_div2[i]), [b] "r"(cplxmul_opBi_div2[i]));

      check_uint32 (result, "pv.cplxmul.h.i.div2", cplxmul_opCi_div2[i],
                    cplxmul_opCResi_div2[i]);
    }
  ...
  for (i = 0; i < N; i++)
    {
      asm volatile(
          "pv.cplxmul.h.i.div4 %[c], %[a], %[b]\n"
          : [c] "+r"(cplxmul_opCi_div4[i])
          : [a] "r"(cplxmul_opAi_div4[i]), [b] "r"(cplxmul_opBi_div4[i]));

      check_uint32 (result, "pv.cplxmul.h.i.div4", cplxmul_opCi_div4[i],
                    cplxmul_opCResi_div4[i]);
    }
  ...
  for (i = 0; i < N; i++)
    {
      asm volatile(
          "pv.cplxmul.h.i.div8 %[c], %[a], %[b]\n"
          : [c] "+r"(cplxmul_opCi_div8[i])
          : [a] "r"(cplxmul_opAi_div8[i]), [b] "r"(cplxmul_opBi_div8[i]));

      check_uint32 (result, "pv.cplxmul.h.i.div8", cplxmul_opCi_div8[i],
                    cplxmul_opCResi_div8[i]);
    }
}
...
void
check_pv_complex (testresult_t *result, void (*start) (), void (*stop) ())
{
  unsigned int i;
  unsigned int res;
  ...
  for (i = 0; i < N; i++)
    {
      ...
      check_uint32 (result, "pv.subrotmj.h.div2", tmp_result[i],
                    subrotmj_opCdiv2[i]);
    }
  ...
  for (i = 0; i < N; i++)
    {
      ...
      check_uint32 (result, "pv.subrotmj.h.div4", tmp_result[i],
                    subrotmj_opCdiv4[i]);
    }
  ...
  for (i = 0; i < N; i++)
    {
      ...
      check_uint32 (result, "pv.subrotmj.h.div8", tmp_result[i],
                    subrotmj_opCdiv8[i]);
    }
