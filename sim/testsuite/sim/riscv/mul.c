...
# output: == test: mulh -> success, nr. of errors: 0\n
# output: == test: mulhu -> success, nr. of errors: 0\n
# output: == test: mulhsu -> success, nr. of errors: 0\n
...
testcase_t testcases[] = { { .name = "mul", .test = check_mul },
...
void
check_mulh (testresult_t *result, void (*start) (), void (*stop) ())
{
  int act;
  int a;
  int b;
  ...
  for (i = 0; i < n_stimuli; i++)
    {
      util_check_mulh (result, stim_mulh_a[i], stim_mulh_b[i],
                       stim_mulh_exp[i]);
    }
}

void
check_mulhu (testresult_t *result, void (*start) (), void (*stop) ())
{
  int act;
  int a;
  int b;
  ...
  for (i = 0; i < n_stimuli; i++)
    {
      util_check_mulhu (result, stim_mulhu_a[i], stim_mulhu_b[i],
                        stim_mulhu_exp[i]);
    }
}

void
check_mulhsu (testresult_t *result, void (*start) (), void (*stop) ())
{
  int act;
  int a;
  int b;
  ...
  util_check_mulhsu (result, 0x2794E000, 0xFFFFFFFF, 0x2794DFFF);

  for (i = 0; i < n_stimuli; i++)
    {
      util_check_mulhsu (result, stim_mulhsu_a[i], stim_mulhsu_b[i],
                         stim_mulhsu_exp[i]);
    }
}
